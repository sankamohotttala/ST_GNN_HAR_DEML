#downsample to 10FPS (from 30FPS) 
#second code to do so it seems 
#  may be the samme code ;
#  at the moment used only for ntu 120-10fps and ntu 60-10fps
import argparse
import pickle
...
sys.path.extend(['../'])
from pre_process_10fps import pre_normalization
...
#                     63,69,80,81,92,95,96,97,98,99,100,101,102,109,113,114] #uses action classes for ntu 44  

#action_classes_used=list(range(1,61)) #for NTU 60
action_classes_used=list(range(1,121)) #for NTU 120
...
def gendata(data_path, out_path,extra_skeleton_path_60=None,extra_skeleton_path_120=None, ignored_sample_path=None, benchmark='xview', part='eval'):
    ...
    TrainSamples=[0 for i in range(44)]
    ValSamples=[0 for i in range(44)]
    for filename in os.listdir(data_path):
        ...
        if benchmark == 'xview':
            ...
            if action_class<61: #previously this was given 60; could have affected other codes as well
                istraining = (subject_id in training_subjects)
            else:
                istraining = (subject_id not in training_subjects)
                #raise ValueError() #just to make sure; for NTU 60 only
...
if __name__ == '__main__':
    ...
    data_path_final=r'F:\Data Sets\Skeleton\3D\Adult\NTU dataset\processed skeleton data\NTU-120-skeleton-full\Data' #for Full NTU 120 dataset
    #data_path_final=r'F:\Data Sets\Skeleton\3D\Adult\NTU dataset\processed skeleton data\Processed skeleton files\nturgbd_skeletons_s001_to_s017\nturgb+d_skeletons' #for Full NTU 60 dataset
    output_folder=r'F:\Codes\joint attention\2022\NTU_to_KinectV1\data_7(10fps)'
