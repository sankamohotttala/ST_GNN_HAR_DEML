...
def pre_normalization(data,out_folder,actionDic,part='__', zaxis=[1, 19], xaxis=[7, 3]):#zaxis=[0, 1], xaxis=[8, 4])
    ...
    print('parallel the bone between hip(jpt 0) and spine(jpt 1) of the first person to the z axis across each frame')
    for i_s, skeleton in enumerate(tqdm(s)): #skeleton : M, T, V, C
        if skeleton.sum() == 0:
            continue
        joint_bottom=[skeleton[0,t,zaxis[0]] for t in range(T)]# coordinates of x,y,z (as seen on the 3d plot)
        joint_top=[skeleton[0,t,zaxis[1]] for t in range(T)]

        axis = [np.cross(joint_top[t] - joint_bottom[t], [0, 0, 1]) for t in range(T)]
        angle = [angle_between(joint_top[t] - joint_bottom[t], [0, 0, 1]) for t in range(T)]
        matrix_z = [rotation_matrix(axis[t], angle[t]) for t in range(T)]
        ...
        for i_p, person in enumerate(skeleton):
            ...
            for i_f, frame in enumerate(person):
                if frame.sum() == 0:
                    continue
                for i_j, joint in enumerate(frame): # goes through all 25 joints
                    s[i_s, i_p, i_f, i_j] = np.dot(matrix_z[i_f], joint)
    # visualization_skeleton_all(s,out_folder,actionDic,part)
    ...
    print(
        'parallel the bone between right shoulder(jpt 8) and left shoulder(jpt 4) of the first person to \
            the x axis across each frame')
    for i_s, skeleton in enumerate(tqdm(s)):
        if skeleton.sum() == 0:
            continue
        # joint_rshoulder = skeleton[0, 0, xaxis[0]]
        joint_rshoulder = [np.copy(skeleton[0, t, xaxis[0]]) for t in range(T)] #if not a copy, the original array is changed later
        joint_lshoulder = [np.copy(skeleton[0, t, xaxis[1]]) for t in range(T)]
        for t in range(T):
            joint_rshoulder[t][2]=0 #if not for the copy, the original array is changed 
            joint_lshoulder[t][2]=0

        axis = [np.cross(joint_rshoulder[t] - joint_lshoulder[t], [1, 0, 0]) for t in range(T)]
        angle = [angle_between(joint_rshoulder[t] - joint_lshoulder[t], [1, 0, 0]) for t in range(T)]
        matrix_x = [rotation_matrix(axis[t], angle[t]) for t in range(T)]
        ...
        for i_p, person in enumerate(skeleton):
            ...
            for i_f, frame in enumerate(person):
                if frame.sum() == 0:
                    continue
                for i_j, joint in enumerate(frame):
                    s[i_s, i_p, i_f, i_j] = np.dot(matrix_x[i_f], joint)
    visualization_skeleton_all(s,out_folder,actionDic,part )
