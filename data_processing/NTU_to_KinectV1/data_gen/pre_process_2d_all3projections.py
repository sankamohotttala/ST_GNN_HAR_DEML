...
def pre_normalization(data,out_folder,actionDic,part='__',projection='xyz', zaxis=[1, 19], xaxis=[7, 3]):#zaxis=[0, 1], xaxis=[8, 4])
    ...
    for i_s, skeleton in enumerate(tqdm(s)):
        ...
        for i_p, person in enumerate(skeleton):
            ...
            aa=s[i_s, i_p] 
            # aa[:,:,1]=0
            plane_index=projectionDic.index(plane)
            aa[:,:,plane_index]=0
            # __=aa.shape
            s[i_s, i_p] =aa
            # s[i_s, i_p] = (s[i_s, i_p] - main_body_center) * mask
