...
from model.stgcn_1_edit import Model, Model2
from model.stgcn_1_edit import INITIALIZERS
...
def reliabilityDigram(dataframe, class_num, epoch_num, bin_n=10):
    ...
    for i in range(class_num):
        ...
        plt.savefig(os.path.join(pathDirectoryRD_epoch,f'reliability_diagram_sklearn_class_id_{CLASS}.png'))
        # plt.show()
        plt.grid('off')
        plt.close()
...
def save_var_new(ind,static_,config,config_2,*argv):#ImplementationName,train_data_path,test_data_path,isStepDecay,trials
    ...
    if static_==True: #static values used in all trials
        for key,val in config_2[1].items():
            tmp+=f'{key}: {val}\n'
...
#config dics
config = {1: {'schedular':'piecewise','decay':0.9,'loss_func':'category_cross','label_smooth_val':0,'scale':2., 'mode':'fan_out', 'dist':'truncated_normal','reg':'l2','reg_valu':0.01},
          2:{'schedular':'piecewise','decay':0.9,'loss_func':'category_cross','label_smooth_val':0.01,'scale':2., 'mode':'fan_out', 'dist':'truncated_normal','reg':'l2','reg_valu':0.01},
          3:{'schedular':'piecewise','decay':0.9,'loss_func':'category_cross','label_smooth_val':0.1,'scale':2., 'mode':'fan_out', 'dist':'truncated_normal','reg':'l2','reg_valu':0.01},
          4:{'schedular':'piecewise','decay':0.9,'loss_func':'category_cross','label_smooth_val':0.2,'scale':2., 'mode':'fan_out', 'dist':'truncated_normal','reg':'l2','reg_valu':0.01},
          5:{'schedular':'piecewise','decay':0.9,'loss_func':'category_cross','label_smooth_val':0.3,'scale':2., 'mode':'fan_out', 'dist':'truncated_normal','reg':'l2','reg_valu':0.01},
          6:{'schedular':'piecewise','decay':0.9,'loss_func':'category_cross','label_smooth_val':0.4,'scale':2., 'mode':'fan_out', 'dist':'truncated_normal','reg':'l2','reg_valu':0.01}
          }

#non looping config
config_2 = {1: { 'target_classes': 10, 'source_cls':60,'proto':'S3','lr': 0.01,'steps':[10,20], 'optimizer':'SGD','batch_size': 4, 'epochs': 30, 'STGCN_OUT':10,'FPS':30}}
...
for ind in range(1,trials+1):
    ...
    scedular_method=config[ind]['schedular'] #piecewise_decay or expo_decay or constant
    ...
    base_lr=config_2[1]['lr'] #0.01
    steps=config_2[1]['steps']
    opti_mode=config_2[1]['optimizer'] #adam or sgd
    # steps=[1,5]

    STGCN_OUT=config_2[1]['STGCN_OUT'] #stgcn output layer - 1 to 10
    PreTrainedClasses=config_2[1]['source_cls'] #since no transfer learning here    
    ChildDatasetClasses=config_2[1]['target_classes'] #change the train, val data paths as well

    ProtocolTraining=config_2[1]['proto'] #S1, S2, S3, S4 ->full, similar, dissimilar, shared
    batchSize=config_2[1]['batch_size']
    ...
    FPS=config_2[1]['FPS'] #either 10 or 30
    #checkpoint_restore_path=None        #this is given a value below

    iteration_dic={'S1':924,'S2':612,'S3':628,'S4':-1} #full, similar, dissimilar, shared
    ...
    used_epochs = config_2[1]['epochs'] #30
    ...
    checkpoint_restore_path=os.path.join(main_checkpoint,checkpoint_name)
    ...
    ImplementationName='/{0}_{1}_{2}_{3}_{4}_{5}_{6}_{7}'.format(PreTrainedClasses,ChildDatasetClasses,LearningMode,ProtocolTraining,scedular_method,base_lr,
    ...
    #save variables
    save_var_new(ind,config_2_static,config,config_2,ImplementationName,train_data_path,test_data_path,isStepDecay,trials)
    ...
    model.load_weights("ntu60")
    
    
    model_fc = Model2(num_classes=num_classes,intializers=initial_model) #child model
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % save_freq == 0: #saves the data to a  csv file
            ...
            reliabilityDigram(df, num_classes, epoch, bin_n=10)
