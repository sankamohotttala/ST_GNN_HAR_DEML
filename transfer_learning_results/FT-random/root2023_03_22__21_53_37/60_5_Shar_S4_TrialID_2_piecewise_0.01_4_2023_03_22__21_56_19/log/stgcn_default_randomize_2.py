...
class Model(tf.keras.Model):
    def __init__(self, num_classes=60,layer_out = 9,initializers=None):
    ...
    def call(self, x, training):
        ...
        return x , -1 , returnLayer


class Model_randomzied(tf.keras.Model):
    def __init__(self, num_classes=60,layer_out = 9,initializers=None):
        super().__init__()
        self.initializers=initializers
        self.prev_out_stgcn=layer_out
        self.num_stgcn = 10 - layer_out #do the rest later ; manually doing the stuff atm
        ...
        self.STGCN_layers = []
        # self.STGCN_layers.append(STGCN(64, residual=False)) #commented because atm input is from the 9th (index 8) stgcn layer
...
class Model2_new(tf.keras.Model):
    def __init__(self, num_classes=10, initializers=None):
        super().__init__()
        self.initializers=initializers
     
        self.dense1=tf.keras.layers.Dense(256, activation='relu',kernel_initializer=self.initializers.INITIALIZER,kernel_regularizer=self.initializers.REGULARIZER) # 512 originally
        self.drop1=tf.keras.layers.Dropout(.5) #.7
        self.dense2=tf.keras.layers.Dense(128, activation='relu',kernel_initializer=self.initializers.INITIALIZER,kernel_regularizer=self.initializers.REGULARIZER)
...
class Model2_single_layer(tf.keras.Model):
    def __init__(self, num_classes=10,initializers=None): 
        super().__init__()
        self.initializers=initializers 

        self.dense3=tf.keras.layers.Dense(num_classes, kernel_initializer=self.initializers.INITIALIZER, kernel_regularizer=self.initializers.REGULARIZER)
