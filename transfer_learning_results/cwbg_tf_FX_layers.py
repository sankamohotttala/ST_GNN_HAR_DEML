...
initial_model=INITIALIZERS(scale=2., mode="fan_out", distribution="truncated_normal",reg='l2',reg_value=0.001) #l2 or l1 for reg;
...
PreTrainedClasses=120 #since no transfer learning here    
ChildDatasetClasses=5 #change the train, val data paths as well
ProtocolTraining='S4' #S1, S2, S3, S4 ->full, similar, dissimilar, shared
batchSize=4
...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % save_freq == 0: #saves the data to a  csv file
            ...
            with open(os.path.join(pathCSV,'data_{}.csv'.format(epoch+1)), 'r') as f:
                reader = csv.reader(f)
