#for layers 0-9 STGCN
#2023 note: this seems to be like a FX code ; gets the output from layer num 5 and then does a FC layer (after pooling)
...
LearningMode='comp' #  TFL-F for "frozen layer transfer learning", TFL-P for "TFL woth propagation method"     
                    #and FX for "feature extraction" and CL for "curriculum learning" and FS for "from sratch"  
FPS=10_300 #either 10 or 30 or 10_300
checkpoint_restore_path="checkpoints/STGCN_44"
iterationNum=500  #originally ST-GCN used 40,000
if ConstantBaseLearnngRate==True:
    iterationNumPrint= -1   #since lr is constant z
else:
    iterationNumPrint=iterationNum

# for CFBG dataset
if ProtocolTraining =='S2':    
    train_data_path = r'E:\SLIIT RA\Weekly stuff\zUltimate_week\CFBG processed datasets\save_directory_2\xsub\train_data' #there are for random 10 classes
    test_data_path  = r'E:\SLIIT RA\Weekly stuff\zUltimate_week\CFBG processed datasets\save_directory_2\xsub\val_data'
elif ProtocolTraining =='S3':
    train_data_path = r'E:\SLIIT RA\Weekly stuff\zUltimate_week\CFBG processed datasets\save_directory_3\xsub\train_data'  #these train val are for the different 10 classes
    test_data_path  = r'E:\SLIIT RA\Weekly stuff\zUltimate_week\CFBG processed datasets\save_directory_3\xsub\val_data'
elif ProtocolTraining =='S1':
    train_data_path = r'E:\SLIIT RA\Weekly stuff\zUltimate_week\CFBG processed datasets\save_directory\xsub\train_data'  #these train val are for the full 15 classes
    test_data_path  = r'E:\SLIIT RA\Weekly stuff\zUltimate_week\CFBG processed datasets\save_directory\xsub\val_data'
elif ProtocolTraining =='S4':
    train_data_path = r'E:\SLIIT RA\Weekly stuff\zUltimate_week\CFBG processed datasets\save_directory_4\xsub\train_data'  #these train val are for the full 15 classes
    test_data_path  = r'E:\SLIIT RA\Weekly stuff\zUltimate_week\CFBG processed datasets\save_directory_4\xsub\val_data'
...
ImplementationName='{0}_test_{1}_class_{5}_iterations_protocol_{4}_{2}FPS_{3}_note_batchnormRemovedOriginal_{6}batchsize{7}'.format(PreTrainedClasses,ChildDatasetClasses,
...
#os.environ["CUDA_VISIBLE_DEVICES"] = "-1" from stack; to train using CPU only
...
def get_parser():
    ...
    parser.add_argument(
        ...
        default=[15, 40],
        nargs='+',
        help='the epoch where optimizer reduce the learning rate, eg: 10 50')#[10, 40]
    parser.add_argument(
        '--gpus',
        default=None,                                                    
        nargs='+',
...
def test_step(features,labels):
    golbal,logits = model(features, training=False)
    logits=model_fc(golbal)
...
def train_step(paramss,features, labels):
  def step_fn(features, labels):
    with tf.GradientTape() as tape:
      golbal,logits = model(features, training=True)# !!!!
...
if __name__ == "__main__":
    ...
    log_dir         = arg.log_dir 
    train_data_path = train_data_path #these train val are for the different 10 classes
    ...
    if ConstantBaseLearnngRate==False: #if true, values won't change
    ...
    with strategy.scope():
        ...
        optimizer    = tf.keras.optimizers.SGD(learning_rate=learning_rate,
                                               momentum=0.9,
                                               nesterov=True)
        # optimizer  = tf.keras.optimizers.Adam(learning_rate=base_lr) # currently this is not used
        ...
        status = ckpt.restore(checkpoint_list[0]) #checkpoint_list[0] or checkpoint_last
    ...
    for epoch in range(epochs):
        ...
        with strategy.scope():
            for features, labels in tqdm(train_data):
                ...
                matrixMatplot.cross_entrophy_loss.append(cross_entropy_loss.result().numpy()) #no ise of this no; no resetting
        ...
        # print(optimizer.lr(optimizer.iterations).numpy()) 
