#for Feature extraction  - test code ; no use since we don't save the FX resultant model LOL
...
initial_model=INITIALIZERS(scale=2., mode="fan_out", distribution="truncated_normal",reg='l1',reg_value=0.001) #pass this to model class constructor
...
loss_fuction='category_cross' #softmax_cross or category_cross ->used for label smoothing
label_smooth_val=0.1 #0.1 is the default value; 0 is when no label smoothing done


#LR and LR schedular selection 
scedular_method='piecewise' #piecewise_decay or expo_decay or constant
...
STGCN_OUT=10 #stgcn output layer - 1 to 10
PreTrainedClasses=60 #since no transfer learning here    
ChildDatasetClasses=10 #change the train, val data paths as well
ProtocolTraining='S3' #S1, S2, S3, S4 ->full, similar, dissimilar, shared
batchSize=4
...
main_checkpoint=r'F:\Codes\joint attention\2022 - Journal\New TF and NTU-44  implementations\TFL_FX_FT_final\_____' #no need for checkpoints
checkpoint_restore_path=os.path.join(main_checkpoint,checkpoint_name)


# for CFBG dataset
if ProtocolTraining =='S1':    #full 
    train_data_path = r'F:\Codes\joint attention\2022\visualize_vattu_child\save_directory\xsub'
    test_data_path  = r'F:\Codes\joint attention\2022\visualize_vattu_child\save_directory\xsub\train_data'
elif ProtocolTraining =='S2': #similar
    train_data_path = r'F:\Codes\joint attention\2022\visualize_vattu_child\save_directory_2\xsub'
    test_data_path  = r'F:\Codes\joint attention\2022\visualize_vattu_child\save_directory_2\xsub\train_data'
elif ProtocolTraining =='S3': #dissimilar
    train_data_path = r'F:\Codes\joint attention\2022\visualize_vattu_child\save_directory_3\xsub'
    test_data_path  = r'F:\Codes\joint attention\2022\visualize_vattu_child\save_directory_3\xsub\train_data'
elif ProtocolTraining =='S4': #shared
    train_data_path = r'F:\Codes\joint attention\2022\visualize_vattu_child\save_directory_4\xsub'
    test_data_path  = r'F:\Codes\joint attention\2022\visualize_vattu_child\save_directory_4\xsub\train_data'
else:
    ValueError()
...
folder_name='results_new/'+ImplementationName
...
if __name__ == "__main__":
    ...
    model        = Model(num_classes=PreTrainedClasses, layer_out=STGCN_OUT) #pre-trained model
    ...
    model_fc = Model2(num_classes=num_classes) #child model
    ...
    for epoch in range(epochs):
        ...
        #                                                         ckpt_save_path))
        if (epoch + 1) % save_freq == 0:
            plotAccuracy(tsne_out,labels_out,False)
    ...
    plotAccuracy(tsne_out,labels_out,True)
