#for TFL fine tuning with randomized layers and frozen/unfrozen layers
#atm keep FC to 1 layer if possible 

from model.stgcn_default_randomize import Model, Model2,Model2_single_layer,Model_randomzied
...
PreTrainedClasses=120 #change the checkpoint folder as well
ChildDatasetClasses=15 #change the train, val data paths as well
ProtocolTraining='S1' #S1 - Full dataset, S2 - Random dataset and S3 - Dissimilar Dataset
batchSize=4
...
checkpoint_restore_path="checkpoints/STGCN_120_10FPS_LS"
...
ImplementationName='{0}_FINETUNE_{1}_class_{5}_iterations_protocol_{4}_{2}FPS_{3}_note_batchnormRemovedOriginal_{6}batchsize{7}'.format(PreTrainedClasses,ChildDatasetClasses,
...
def test_step(features,labels):
    ...
    logits = model_random(logits,training=False)
...
@tf.function
def test_step_FC(features,labels): #need to add the 2nd model
    logits , _ = model(features, training=False)
    logits = model_random(logits,training=False)
    logits=model_fc(logits, training=False)
    # cross_entropy = tf.nn.softmax_cross_entropy_with_logits(logits=logits,
...
def train_step(paramss,features, labels):
  def step_fn(features, labels):
    with tf.GradientTape() as tape:
      logits , _ = model(features, training=False)
      logits = model_random(logits,training=True)
    ...
    grads = tape.gradient(loss, model_fc.trainable_variables + model.trainable_variables + model_random.trainable_variables) 
    '''
    for _ in model.trainable_variables:
        print('trainable ',_)
    for _ in model.layers:
        print('all_layers',_.name, _.trainable)
    for _ in model.non_trainable_variables:
        print('non-trainable ',_)
    '''
    optimizer.apply_gradients(list(zip(grads, model_fc.trainable_variables + model.trainable_variables + model_random.trainable_variables)))
...
@tf.function #need to add the 2nd model
def train_step_FC(paramss,features, labels):
  def step_fn(features, labels):
    with tf.GradientTape() as tape:
      logits , _ = model(features, training=True)# traiing =False???
      logits = model_random(logits,training=True)
...
if __name__ == "__main__":
    ...
    with strategy.scope():
        ...
        status = ckpt.restore(checkpoint_last) #checkpoint_list[0]

        model_random = Model_randomzied(num_classes=PreTrainedClasses)
        
        model_fc = Model2_single_layer(num_classes=ChildDatasetClasses) #try with 'Model2'
    ...
    #may be better to not do this with randomized layers being present..
    ...
    a=Model_randomzied.layers #NO USE
    a=len(model.layers)
    print(model.layers[10].name)
    # for layer in model.layers[1].layers:
    #     print(layer.name, layer)
    istranable= model.layers[1].trainable
    print("----------------Layers of the Model() - Main ST-GCN model----------------------------")
    for layer in model.layers:
        print(layer.name,layer )
    ...
    #when unfreezing some layer, only do that to those layers 
    print("----------------End Main ST-GCN model----------------------------")
    ...
    for layer in model_random.layers:
        ...
        #need to change the architecture --- why???
    

    print("----------------End Second ST-GCN model----------------------------")

    print('all layers are set to Trainable = False at this point, Do the necesary changes now ')
    print('at this point we have all layers of random model trainable and model_fc as trainable(by default) and some of the layers of original stgcn model frozen')
    print('Need to be careful about STGCN layer where change happens;same parametre should be given in both Model and random_Model')
