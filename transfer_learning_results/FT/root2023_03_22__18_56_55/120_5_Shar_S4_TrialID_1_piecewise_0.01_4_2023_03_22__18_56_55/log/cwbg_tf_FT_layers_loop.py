...
from model.stgcn_default_changed_2 import Model, Model2, Model2_single_layer
from model.stgcn_default_changed_2 import INITIALIZERS
...
def save_var_new(ind,static_,config,config_2,*argv):#ImplementationName,train_data_path,test_data_path,isStepDecay,trials
    ...
    if static_==True: #static values used in all trials
        for key,val in config_2[use_ind].items():
            tmp+=f'{key}: {val}\n'
...
#config dics
config = {1: {'schedular':'piecewise','decay':0.90,'loss_func':'category_cross','label_smooth_val':0,'scale':2., 'mode':'fan_out', 'dist':'truncated_normal','reg':'l2','reg_valu':0.001,'epochs_fc':3},
          2: {'schedular':'expo','decay':0.90,'loss_func':'category_cross','label_smooth_val':0,'scale':2., 'mode':'fan_out', 'dist':'truncated_normal','reg':'l2','reg_valu':0.001,'epochs_fc':3},
          3: {'schedular':'expo','decay':0.94,'loss_func':'category_cross','label_smooth_val':0.1,'scale':2., 'mode':'fan_out', 'dist':'truncated_normal','reg':'l2','reg_valu':0.001,'epochs_fc':3},
          4: {'schedular':'expo','decay':0.92,'loss_func':'category_cross','label_smooth_val':0.1,'scale':2., 'mode':'fan_out', 'dist':'truncated_normal','reg':'l2','reg_valu':0.001,'epochs_fc':3},
          5: {'schedular':'expo','decay':0.92,'loss_func':'category_cross','label_smooth_val':0.1,'scale':2., 'mode':'fan_out', 'dist':'truncated_normal','reg':'l2','reg_valu':0.001,'epochs_fc':3},
          6: {'schedular':'expo','decay':0.92,'loss_func':'category_cross','label_smooth_val':0.1,'scale':2., 'mode':'fan_out', 'dist':'truncated_normal','reg':'l2','reg_valu':0.001,'epochs_fc':3}
          }

#non looping config
config_2 = {1: { 'target_classes': 15, 'source_cls':120,'proto':'S1','lr': 0.01,'steps':[15,25], 'optimizer':'SGD','batch_size': 4, 'epochs': 30, 'S':10, 'E':11, 'FPS':30},
            2: { 'target_classes': 10, 'source_cls':120,'proto':'S2','lr': 0.01,'steps':[15,25], 'optimizer':'SGD','batch_size': 4, 'epochs': 30, 'S':10, 'E':11, 'FPS':30},
            3: { 'target_classes': 10, 'source_cls':120,'proto':'S3','lr': 0.01,'steps':[15,25], 'optimizer':'SGD','batch_size': 4, 'epochs': 30, 'S':10, 'E':11, 'FPS':30},
            4: { 'target_classes': 5, 'source_cls':120,'proto':'S4','lr': 0.01,'steps':[15,25], 'optimizer':'SGD','batch_size': 4, 'epochs': 30, 'S':10, 'E':11, 'FPS':30}
            }
# config_2 = {1: { 'target_classes': 5, 'source_cls':120,'proto':'S4','lr': 0.01,'steps':[10,20], 'optimizer':'SGD','batch_size': 4, 'epochs': 30, 'S':10, 'E':11, 'FPS':30},
#             2: { 'target_classes': 5, 'source_cls':60,'proto':'S4','lr': 0.01,'steps':[10,20], 'optimizer':'SGD','batch_size': 4, 'epochs': 30, 'S':10, 'E':11, 'FPS':30},
#             3: { 'target_classes': 5, 'source_cls':22,'proto':'S4','lr': 0.01,'steps':[10,20], 'optimizer':'SGD','batch_size': 4, 'epochs': 30, 'S':10, 'E':11, 'FPS':30},
#             4: { 'target_classes': 5, 'source_cls':44,'proto':'S4','lr': 0.01,'steps':[10,20], 'optimizer':'SGD','batch_size': 4, 'epochs': 30, 'S':10, 'E':11, 'FPS':30},
#             5: { 'target_classes': 5, 'source_cls':60,'proto':'S4','lr': 0.01,'steps':[10,20], 'optimizer':'SGD','batch_size': 4, 'epochs': 30, 'S':10, 'E':11, 'FPS':30},
#             6: { 'target_classes': 5, 'source_cls':120,'proto':'S4','lr': 0.01,'steps':[10,20], 'optimizer':'SGD','batch_size': 4, 'epochs': 30, 'S':10, 'E':11, 'FPS':30}            
#             }
config_2_static = True      #True if we are using the first instance in config_2 for all trials; False if we have similar instances in config_2 as in config
use_ind=4 # if above True, then use this index from config_2


#we should do experiment across CWBG protocols;not across  target datasets
root_folder='results_loop/'+'root'+ util.data_time_string()
save_all_data=True
skeleton_structure='coco' #need to probably change the graph as well

weight_path=r'F:\Codes\joint attention\2022 - Journal\New TF and NTU-44  implementations\Create weights\save_weights\NTU 120\CKPT 6\ntu120' #last ntu120 is necessary
# weight_path_config = {1:r'F:\Codes\joint attention\2022 - Journal\New TF and NTU-44  implementations\Create weights\save_weights\NTU 120\ntu120',
...
for ind in range(1,trials+1):
    ...
    def test_step(features,labels,loss_fn='softmax_cross',label_smooth=0.01):
        __ , gap_embed, inter_logits = model(features, training=False)
        logits=model_fc(inter_logits) #notice if we are to add drop out we have to pass training=True and change the code in model_fc
    ...
    #   def step_fn(features, labels):
        with tf.GradientTape() as tape:
            ...
            if loss_fn=='softmax_cross':
                ...
                raise NotImplementedError
            loss = tf.reduce_sum(cross_entropy) * (1.0 / global_batch_size)
        grads = tape.gradient(loss, model.trainable_variables + model_fc.trainable_variables)

        optimizer.apply_gradients(list(zip(grads, model.trainable_variables + model_fc.trainable_variables)))  
    ...
    model        = Model(num_classes=PreTrainedClasses, initializers=initial_model) #pre-trained model
    ...
    model_fc = Model2_single_layer(num_classes=num_classes,intializers=initial_model) #child model
