#code is taken from the main_tf_from_scratch.py
#only to be used with child data


from model.stgcn_without_change import Model
...
save_all_data=False
skeleton_structure='body_25' # others are  'coco' and 'no_foot';
# ALSO remember to change the imported graph in stgcn model!!!!

ConstantBaseLearnngRate=False #if true then base_lr will be same ;if False LRschedular will be used as in the code
base_lr=1e-1 #1e-3
steps=[15,25] #[20,40]

PreTrainedClasses=3 #since no transfer learning here
ChildDatasetClasses=3 #change the train, val data paths as well
ProtocolTraining='BODY_25_child_3' #since there is only one
batchSize=4
...
used_epochs = 1 #70
checkpoint_restore_path='checkpoints/Class3_1_BODY_25'
# for CFBG dataset
...
ImplementationName='{0}_{1}_{5}_{4}_{2}_{3}_{11}_{6}_{7}_{8}_{9}_{10}'.format(PreTrainedClasses,ChildDatasetClasses,FPS,LearningMode,ProtocolTraining,iterationNumPrint,
                                                                        batchSize,datatime,ConstantBaseLearnngRate,base_lr,steps,note) #change this every time the implementation is changed
...
folder_name='images_new/'+ImplementationName
...
def train_step(paramss,features, labels):
  def step_fn(features, labels):
    ...
    grads = tape.gradient(loss, model.trainable_variables) #do we have to have order (i.e., layers order) in training variable list??
...
def save_values_csv(predictions,labels,features_data,epoch):
    ...
    #no return : we save the values to a csv file
...
if __name__ == "__main__":
    ...
    with strategy.scope():
        ...
        status = ckpt.restore(checkpoint_last) #checkpoint_last
        ...
        for epoch in range(epochs_FC):
            ...
            with strategy.scope():
                for features, labels in tqdm(train_data):
                    features =util.randomSelection(features,random_choose=True,window_size=150,random_move=True)
    ...
    # S=1 #this was used in all propagation thing before
    
    # S=1 #layer index S is the start ofthe unfreezing thus S->[1,11] when 10 final stgcn layer unfrozen when 11 non is unrozen
