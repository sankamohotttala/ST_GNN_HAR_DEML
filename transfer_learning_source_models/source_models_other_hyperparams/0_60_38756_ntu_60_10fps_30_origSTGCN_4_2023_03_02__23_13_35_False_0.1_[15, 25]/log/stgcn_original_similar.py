...
REGULARIZER = tf.keras.regularizers.l2(l=0.0001) #0.0001 is the default value
...
class STGCN(tf.keras.Model):
    def __init__(self, filters, kernel_size=[9, 3], stride=1, activation='relu',
        ...
        self.tgcn.add(tf.keras.layers.Dropout(.5))
...
class Model(tf.keras.Model):
    def __init__(self, num_classes=60):
        ...
        if edge_importance_weighting:
            self.all_edge_importance = [tf.Variable(tf.ones([graph.A.shape[1],graph.A.shape[1]]),
                                               dtype=tf.float32,
                                               trainable=True ) for _ in range(len(self.STGCN_layers))]
        else:
            self.all_edge_importance = 1
    ...
    def call(self, x, training):
        ...
        for layer in self.STGCN_layers:
            tmp = A * self.all_edge_importance[id]
            x, A_ = layer(x, tmp, training=training)#check if A changes inside ie learns something
            id+=1
