...
#later added: label smoothing as an option with the loss function
...
tsne_fig_size=(25,25) # for small: (10,10) , for large: (25,25)
...
@tf.function
def test_step(features,labels,loss_fn='softmax_cross'):
    logits , gap_embed = model(features, training=False)
    if loss_fn=='softmax_cross':
        cross_entropy = tf.nn.softmax_cross_entropy_with_logits(logits=logits,
                                                              labels=labels)
    elif loss_fn=='category_cross':
        cross_entropy=tf.keras.losses.CategoricalCrossentropy(from_logits=True,label_smoothing=0.2,
...
def train_step(features, labels,loss_fn='softmax_cross'):
...
if __name__ == "__main__":
    ...
    train_step(features, labels, loss_fn=loss_fuction)
    with summary_writer.as_default():
      tf.summary.trace_export(name="training_trace",step=0)
    tf.summary.trace_off()
    
    tf.summary.trace_on(graph=True)
    test_step(features,labels,loss_fn=loss_fuction)
    with summary_writer.as_default():
    ...
    for epoch in range(epochs):
        ...
        for features, labels in tqdm(test_data):
            y_pred ,gap_embed = test_step(features,labels,loss_fn=loss_fuction)
