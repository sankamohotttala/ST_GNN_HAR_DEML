...
ChildDatasetClasses=400 #change the train, val data paths as well
ProtocolTraining='sample_400' #since there is only one
batchSize=4
...
def save_values_csv(predictions,labels,features_data,epoch):
    ...
    for i in range(batch_size):
        ...
        ind_pred=np.argmax(predictions[i])
        ind_label=np.argmax(labels[i]) #from 0 to n-1
        probabilityArray=list(predictions[i].numpy())
        max_probability=np.amax(predictions[i])
        assert predictions[i][ind_pred]== max_probability

        dataTmp.extend([uniqueID,ind_label,ind_pred,max_probability]+probabilityArray)
...
if __name__ == "__main__":
    ...
    csvHeader=['uniqueID','label','calssified_class','probability']+classLabelCSV
