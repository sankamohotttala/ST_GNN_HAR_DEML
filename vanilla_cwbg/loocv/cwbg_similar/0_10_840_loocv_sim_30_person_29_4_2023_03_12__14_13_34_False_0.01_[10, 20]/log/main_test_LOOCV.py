...
for child in range(num_children): #child from 0 to 29
    ...
    ProtocolTraining='loocv_sim' #since there is only one
    batchSize=4
    ...
    iterationNum=840 #cwbg all -  , similar - 840 , dissimilar - 868, shared -432
    ...
    folder_name='results_cwbg_new_protocol/loocv_si_1/'+ImplementationName #root folder also gets created 
    logDirectory=folder_name+'/log'
    checkpoint_restore_path=os.path.join(r'F:\Codes\joint attention\2022 - Journal\zST-GCN_CWBG_LOOP\checkpoints\loocv_si_1',ImplementationName)
