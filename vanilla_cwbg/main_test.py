#used for st-gcn implementations with cwbg all 4 protocols
#used for repeated stgcn implmentations as well; 
#note we need to create the root checkpoint and results folders [sure?] 
...
#still iteration wise test loss show odd behavior eg: starting with really high values like 100x or 1000x
#used this code along with stgcn_cwbg.py in model folder to experimentwith other initializers and l2 regularizers
...
for i in range(num_trials):
    ...
    ProtocolTraining='cwbg_shared' #since there is only one
    batchSize=4
    ...
    iterationNum=312 #cwbg all - 924 , similar - 612, dissimilar - 628, shared -312
    ...
    # for CFBG dataset
    if ProtocolTraining=='cwbg_similar':    
    ...
    elif ProtocolTraining=='cwbg_dissimilar':
    ...
    elif ProtocolTraining=='cwbg_shared':
    ...
    folder_name='results_cwbg_loop/cwbg_shared_exdec/'+ImplementationName
    logDirectory=folder_name+'/log'
    checkpoint_restore_path=os.path.join(r'F:\Codes\joint attention\2022 - Journal\zST-GCN_CWBG_LOOP\checkpoints\cwbg_shared_exdec',ImplementationName)
