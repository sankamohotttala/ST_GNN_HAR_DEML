#used for st-gcn implementations with new train-test age protocols [only age]
#used for repeated stgcn implmentations as well; note we need to create the root checkpoint and results folders
#added the scheduler with exponential decay as well - can selecetexpo decay or piecewise step
#changed the matplotlib backend to stop failed bitmap error
#resolved the test loss linear increase issue
#use this code along with stgcn_cwbg.py in model folder to experimentwith other initializers and l2 regularizers
...
def getnew_dataset(train_data_tmp,mode=None):

    feature_tensor_list=[]
    label_tensor_list=[]
    # name_tensor_list=[]

    for features, labels in train_data_tmp:
    ...
    dataset=tf.data.Dataset.from_tensor_slices((feature_tensor_f,label_tensor_f))
...
for i in range(num_trials):
    ...
    ProtocolTraining='random_shared' #since there is only one
    batchSize=4

    datatime=util.data_time_string() #used as a unique number as well so we know when the code was ran
    LearningMode='from_scratch'  #without_strategy_testCross
    FPS=30 #either 10 or 30
    #checkpoint_restore_path=None        #this is given a value below
    iterationNum=312 #train boys:652 , #cwbg all - 924 , similar - 612, dissimilar - 628, shared -312
    ...
    # for CFBG dataset
    if ProtocolTraining=='random_full':    
        train_data_path = r'F:\Codes\joint attention\2022\visualize_vattu_child\new_protocols\random_split\full\xsub\train_data'
        test_data_path  = r'F:\Codes\joint attention\2022\visualize_vattu_child\new_protocols\random_split\full\xsub\val_data'
    elif ProtocolTraining=='random_dissimilar':
        train_data_path = r'F:\Codes\joint attention\2022\visualize_vattu_child\new_protocols\random_split\dissimilar\xsub\train_data' 
        test_data_path  = r'F:\Codes\joint attention\2022\visualize_vattu_child\new_protocols\random_split\dissimilar\xsub\val_data'
    elif ProtocolTraining=='random_similar':
        train_data_path = r'F:\Codes\joint attention\2022\visualize_vattu_child\new_protocols\random_split\similar\xsub\train_data'
        test_data_path  = r'F:\Codes\joint attention\2022\visualize_vattu_child\new_protocols\random_split\similar\xsub\val_data'
    elif ProtocolTraining=='random_shared':
        train_data_path = r'F:\Codes\joint attention\2022\visualize_vattu_child\new_protocols\random_split\shared\xsub\train_data'
        test_data_path  = r'F:\Codes\joint attention\2022\visualize_vattu_child\new_protocols\random_split\shared\xsub\val_data'  
    else:
        ValueError()
    ...
    folder_name='results_randomSplit/shared/'+ImplementationName
    logDirectory=folder_name+'/log' 
    checkpoint_restore_path=os.path.join(r'F:\Codes\joint attention\2022 - Journal\zST-GCN_CWBG_LOOP\checkpoints\ran_shared',ImplementationName)
...
#-----------------
    ...
    #get new dataset for speed - not using since there is no increase in speed
    # train_data=getnew_dataset(train_data,'train')
    # test_data=getnew_dataset(test_data,'test')
