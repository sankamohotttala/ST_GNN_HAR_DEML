...
for child in range(num_children): #child from 0 to 29
    ...
    ProtocolTraining='loocv_diss' #since there is only one
    batchSize=4
    ...
    iterationNum=868 #cwbg all - 924 , similar - 612, dissimilar - 628, shared -312
    ...
    folder_name='results_cwbg_new_protocol/loocv_diss_2/'+ImplementationName #root folder also gets created 
    logDirectory=folder_name+'/log'
    checkpoint_restore_path=os.path.join(r'F:\Codes\joint attention\2022 - Journal\zST-GCN_CWBG_LOOP\checkpoints\loocv_di_2',ImplementationName)
