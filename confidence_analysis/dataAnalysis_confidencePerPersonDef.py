...
saveCsvPath=r'F:\Codes\joint attention\2022 - comparison paper\kineticsSTGCN_code\result_analysis\Results_save\realKinetics400_multiperson.csv'

finalCsvPath=r'F:\Codes\joint attention\2022 - comparison paper\kineticsSTGCN_code\result_analysis\results_exp\realKinetics400_multiperson_train.csv'
...
def main():
    ...
    average_conf_P_0,average_conf_P_1=calculateConfidenceAverage(numpyData,pickleData,methodUsed='default')
    average_conf_P_0_frame,average_conf_P_1_frame=calculateConfidenceAverage(numpyData,pickleData,methodUsed='frame_based')  
    ...
    final_average_conf_p_0=averageConfidenceRepresent(average_conf_P_0,acc_dic,dicMethod='rearrange')
    final_average_conf_p_1=averageConfidenceRepresent(average_conf_P_1,acc_dic,dicMethod='rearrange')

    final_average_conf_frame_p_0=averageConfidenceRepresent(average_conf_P_0_frame,acc_dic,dicMethod='rearrange')
    final_average_conf_frame_p_1=averageConfidenceRepresent(average_conf_P_1_frame,acc_dic,dicMethod='rearrange')

    # result_dataframe=createDatadrame_PerSkeleton(final_average_conf_p_0,final_average_conf_p_1, acc_dic, class_name_list) #only for single one; default is used
    result_dataframe=createDatadrame_PerSkeletonMultiple(final_average_conf_p_0,final_average_conf_p_1,final_average_conf_frame_p_0,final_average_conf_frame_p_1, acc_dic, class_name_list) # default+frame is used
...
def createDatadrame_PerSkeleton(final_average_conf,final_average_conf_frame, acc_dic, class_name_list):         
    df_joint_first=pd.DataFrame.from_dict(final_average_conf, orient='index', columns=['default_confidence_P0'])
    df_joint_second=pd.DataFrame.from_dict(final_average_conf_frame, orient='index',columns=['default_confidence_P1'])
    #add two dataframes horizontally
    df_joint=pd.concat([df_joint_first,df_joint_second],axis=1)
...
def createDatadrame_PerSkeletonMultiple(default_p0,default_p1,frame_p0,frame_p1, acc_dic, class_name_list):         
    df_joint_first=pd.DataFrame.from_dict(default_p0, orient='index', columns=['default_confidence_P0'])
    df_joint_second=pd.DataFrame.from_dict(default_p1, orient='index',columns=['default_confidence_P1'])
    df_joint_third=pd.DataFrame.from_dict(frame_p0, orient='index', columns=['frame_confidence_P0'])
    df_joint_fourth=pd.DataFrame.from_dict(frame_p1, orient='index',columns=['frame_confidence_P1'])
    #add two dataframes horizontally
    df_joint=pd.concat([df_joint_first,df_joint_second,df_joint_third,df_joint_fourth],axis=1)
    df_joint_first_final=df_joint.reset_index(drop=True) #remove the index and drop it so this and others can be combined
...
def calculateConfidenceAverage(data,pickleData,methodUsed='default'):
    ...
    accumulated_conf_dict={str(i):0 for i in range(number_class)} #for person 0
    accumulated_conf_dict_1={str(i):0 for i in range(number_class)} #for person 1
    ...
    for i in range(numpyData.shape[0]):
        ...
        if numpyData[i,2,:,:,0].sum()==0 or numpyData[i,2,:,:,1].sum()==0: 
            TT+=1               #just to see if this matters
            add=False
        # if numpyData[i,2,:,:,:].sum()==0: #== to sum() entire numpyArray
        #     add=False

        if methodUsed=='default':
            sequence_average_conf=numpyData[i,2,:,:,0].sum()/(300*18)
            sequence_average_conf_1=numpyData[i,2,:,:,1].sum()/(300*18)
            add=True
            ...
            for i in range(data.shape[0]):
                for j in range(data.shape[1]):
                    ...
                    if data[i,j,1]==0:
                        tmp_1+=1
            if add:
                sequence_average_conf=numpyData[i,2,:,:,0].sum()/(300*18-tmp)
                sequence_average_conf_1=numpyData[i,2,:,:,1].sum()/(300*18-tmp_1)
        ...
        if methodUsed=='frame_based':
            ...
            for i in range(data.shape[0]):
                if data[i,:,0].sum()==0:
                    tmp+=1  #number of missing frames
                if data[i,:,1].sum()==0:
                    tmp_1+=1
            if add:
                sequence_average_conf=numpyData[i,2,:,:,0].sum()/(18*(300-tmp))
                sequence_average_conf_1=numpyData[i,2,:,:,1].sum()/(18*(300-tmp_1))

        if add:
            distri_dict[str(label)]+=1
            accumulated_conf_dict[str(label)]+=sequence_average_conf #person 0
            accumulated_conf_dict_1[str(label)]+=sequence_average_conf_1 #person 1

    average_conf_dict={str(i):(accumulated_conf_dict[str(i)]/distri_dict[str(i)]) for i in range(number_class)}
    average_conf_dict_1={str(i):(accumulated_conf_dict_1[str(i)]/distri_dict[str(i)]) for i in range(number_class)}
    return average_conf_dict, average_conf_dict_1
...
def averageConfidenceRepresent(average_conf,acc_dic,dicMethod='rearrange'):    
    ...
    # if dicMethod=='sort':  #have to rewriet the sort for multiple skeletons
